...
export const OrdersPage = () => {
  ...
  const handleAutoCancellation = async () => {
    ...
    try {
      ...
      // Refresh orders and stats if any were cancelled
  ...
  const columns = [
    ...
    { 
      ...
      renderCell: (params) => (
        <Box>
          <Typography variant="body2" fontWeight="500">
            {params.row?.deliveryAddress?.name || 'N/A'}
          </Typography>
    ...
    { 
      ...
      renderCell: (params) => (
        <Typography variant="body2" fontWeight="600" color="primary">
          ₹{params.row?.pricing?.total || params.row?.total || 0}
        </Typography>
      )
    },
    { 
      ...
      renderCell: (params) => (
        <Box>
          <Typography variant="caption" display="block" textTransform="uppercase">
            {params.row?.payment?.method || 'N/A'}
          </Typography>
          <Chip
            label={params.row?.payment?.status || 'pending'}
            size="small"
            sx={{
              fontSize: '0.7rem',
              height: '18px',
              backgroundColor: params.row?.payment?.status === 'completed' ? '#4caf50' : '#ff9800',
              color: 'white'
            }}
          />
        </Box>
    ...
    { 
      ...
      renderCell: (params) => (
        <Chip
          ...
          size="small"
          sx={{
            backgroundColor: statusColors[params.value] || '#666',
            color: 'white',
            fontWeight: 500,
    ...
    <Box>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
        ...
        <Button
          ...
          startIcon={autoCancelLoading ? <CircularProgress size={20} /> : null}
        >
          {autoCancelLoading ? 'Processing...' : 'Auto-Cancel Overdue Orders'}
        </Button>
      </Box>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        ...
        <Grid item xs={6} sm={4} md={2}>
          <Card>
            <CardContent>
              <Typography variant="h6" sx={{ color: statusColors.pending }}>{stats.pending}</Typography>
        ...
        <Grid item xs={6} sm={4} md={2}>
          <Card>
            <CardContent>
              <Typography variant="h6" sx={{ color: statusColors.processing }}>{stats.processing}</Typography>
        ...
        <Grid item xs={6} sm={4} md={2}>
          <Card>
            <CardContent>
              <Typography variant="h6" sx={{ color: statusColors.shipped }}>{stats.shipped}</Typography>
              <Typography variant="body2" color="textSecondary">Shipped</Typography>
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={6} sm={4} md={2}>
          <Card>
            <CardContent>
              <Typography variant="h6" sx={{ color: statusColors.delivered }}>{stats.delivered}</Typography>
        ...
        <Grid item xs={6} sm={4} md={2}>
          <Card>
            <CardContent>
              <Typography variant="h6" sx={{ color: statusColors.cancelled }}>{stats.cancelled}</Typography>
      ...
      <Box sx={{ height: 600, width: '100%' }}>
        <DataGrid 
          rows={Array.isArray(orders) ? orders.map((o) => ({ ...o, id: o._id || o.orderId || Math.random() })) : []} 
          columns={columns} 
